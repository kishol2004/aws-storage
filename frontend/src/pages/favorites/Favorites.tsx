...
import { Heart, FileText, Star, AlertCircle, Eye, Loader2 } from 'lucide-react';
...
export const Favorites: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['favorites'],
    queryFn: () => documentsApi.list({ isFavorite: true }).then((r) => r.data),
  });
  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Documents you've starred for quick access</p>
      </div>
      ...
      {error && (
        <div className="flex items-center gap-3 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-700">
          ...
          Backend not connected. Favorites will appear once your API is configured.
        </div>
      )}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
        {isLoading ? (
          ...
          <div className="py-24 flex flex-col items-center text-center">
            ...
            <p className="text-sm font-semibold text-slate-700 mb-1">No favorites yet</p>
            <p className="text-xs text-slate-400">Star a document in All Files to see it here</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-100">
            {docs.map((doc) => {
              ...
              return (
                ...
                >
                  <div className="h-9 w-9 rounded-xl bg-amber-50 border border-amber-100 flex items-center justify-center flex-shrink-0 group-hover:bg-amber-100 transition-colors">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      className="btn-ghost rounded-xl px-3 py-1.5 text-xs font-semibold text-violet-600 hover:bg-violet-50 flex items-center gap-1.5 transition-colors"
                      title="View file"
                    >
                    ...
                    <button
                      onClick={() => unstarMutation.mutate(doc.documentId)}
                      title="Remove from favorites"
                      className="text-amber-400 hover:text-slate-300 transition-colors p-1.5 rounded-lg hover:bg-slate-100"
                    >
